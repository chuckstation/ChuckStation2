...
namespace chuckstation2::settings {

void print_version() {
    puts(
        "ChuckStation2 (" STR(_CS2_VERSION) " " STR(_CS2_OSVERSION) ")\n"
...
void print_help() {
    puts(
        "Usage: ChuckStation2 [OPTION]... <path-to-disc-image>\n"
        "\n"
...
bool parse_mappings_file(chuckstation2::instance* iris) {
    ...
    if (!result) {
        ...
        printf("ChuckStation2: Couldn't parse mappings file: %s\n", desc.c_str());
...
bool parse_toml_settings(chuckstation2::instance* iris) {
    ...
    if (!result) {
        ...
        printf("ChuckStation2: Couldn't parse settings file: %s\n", desc.c_str());
    ...
    iris->screenshot_format = screenshots["format"].value_or(CS2_SCREENSHOT_FORMAT_PNG);
    iris->screenshot_jpg_quality_mode = screenshots["jpg_quality_mode"].value_or(CS2_SCREENSHOT_JPG_QUALITY_MAXIMUM);
    iris->screenshot_jpg_quality = screenshots["jpg_quality"].value_or(50);
    iris->screenshot_mode = screenshots["mode"].value_or(CS2_SCREENSHOT_MODE_INTERNAL);
    ...
    auto ui = tbl["ui"];
    iris->theme = ui["theme"].value_or(CS2_THEME_GRANITE);
    iris->codeview_font_scale = ui["codeview_font_scale"].value_or(1.0f);
    iris->codeview_color_scheme = ui["codeview_color_scheme"].value_or(CS2_CODEVIEW_COLOR_SCHEME_SOLARIZED_DARK);
...
#ifdef _WIN32
    iris->windows_titlebar_style = tbl["ui"]["windows_titlebar_style"].value_or(CS2_TITLEBAR_DEFAULT);
...
void parse_cli_settings(chuckstation2::instance* iris, int argc, const char* argv[]) {
...
void close(chuckstation2::instance* iris) {
    ...
    file << "# File auto-generated by " CS2_TITLE "\n\n";
    ...
    mappings_file << "# File auto-generated by " CS2_TITLE "\n\n";
